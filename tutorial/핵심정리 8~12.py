...
not       a not b       NOT(논리 부정), 참과 거짓을 뒤집음
"""
...
# 여러 줄로 된 문자열
"""
여러 줄로 된 문자열은 작은따옴표 3개로 시작하고 작은따옴표 3개로 닫거나
큰 따옴표 3개로 시작해서 큰따옴표 3개로 닫아서 표현합니다.
"""
'''Hello world!
안녕하세요.
Python입니다.'''
...
# 리스트
"""
리스트는 여러 개의 값(요소)을 일렬로 늘어놓은 형태입니다. 변수에 값을 저장할 때 [](대괄호)로 묶어주면 리스트가 되며 각 값은 ,(콤마)로 구분합니다.
리스트에 저장된 요소에 접근할 때는 [] 안에 인덱스를 지정해 줍니다. 특히 리스트의 인덱스는 0부터 시작합니다.
...
# 튜플
"""
튜플은 여러 개의 값(요소)을 일렬로 늘어놓은 형태입니다. 단, 요소의 값을 변경하거나 추가할 수 없습니다(읽기 전용). 
변수에 값을 저장할 때 ( )(괄호)로 묶어주면 튜플이 되며 각 값은 콤마로 구분합니다. 또는, 괄호로 묶지 않고 값만 콤마로 구분해도 튜플이 됩니다. 
튜플에 저장된 요소에 접근할 때는 [ ]안에 인덱스를 지정해줍니다. 그리고 리스트와 마찬가지로 튜플의 인덱스도 0부터 시작합니다.
...
# 시퀀스 자료형
"""
리스트(list), 튜플(tuple), range, 문자열(str)과 같이 값이 연속적으로 이어진 자료형을 시퀀스 자료형(sequence types)이라고 합니다. 
그리고 시퀀스 자료형으로 만든 객체를 시퀀스 객체라고 하며, 시퀀스 객체에 들어있는 각 값을 요소(element)라고 부릅니다.
"""

# 시퀀스 자료형의 공통 기능
"""
시퀀스 자료형은 공통된 동작과 기능을 제공합니다. 따라서 리스트, 튜플, range, 문자열 등의 시퀀스 자료형은 같은 문법을 사용합니다.
