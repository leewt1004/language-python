...
설명
    ...
    스택(Stack) 방식의 함수 호출
    - 파이썬에서 함수를 호출할 때는 스택(Stack)이라는 구조를 사용합니다.
    - 새로운 함수가 호출될 때마다 스택의 아래쪽으로 함수가 쌓이다고 생각할 수 있습니다.
    - 함수가 끝나면 가장 위에 있는 함수부터 차례로 스택에서 사라지게 됩니다.
    - 즉, 프로그램은 함수를 호출할 때마다 스택에 쌓아두고, 나중에 호출된 함수가 먼저 실행을 끝내고, 처음 호출된 함수는 맨 마지막에 끝나는 방식으로 진행됩니다.

    요약
    - 전역 프레임 : 전체 코드에서 접근 가능한 변수와 함수가 저장되는 공간입니다.
    - 스택 프레임 : 함수가 호출될 때마다 생성되는 임시 저장 공간으로, 함수가 끝나면 사라집니다.
    - 함수는 스택구조로 호출되어, 새로운 함수가 호출될 때마다 아래로 쌓이고, 함수가 끝날 때마다 위에서 하나씩 사라집니다.
    - 스택 구조를 컵을 쌓는 것으로 생각하면 쉽습니다. ---> 함수 호출 = 컵을 쌓기 / 함수 끝내기 = 컵을 제거하기
        · add 함수가 호출되면 첫 번째 컵 쌓기
        · add 함수 안에서 mul 함수가 호출되면, 두 번째 컵 쌓기
        · mul 함수가 끝나면 가장 위에 있는 mul 컵 치우기
        · add 함수가 끝나면 그 아래에 있는 add 컵 치우기
    - 결론적으로 새로운 함수가 호출되면 컵을 위에 쌓고, 함수가 끝나면 위에서부터 하나씩 치우는 구조
"""
